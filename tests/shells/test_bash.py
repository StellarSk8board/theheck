...
class TestBash(object):
    ...
    def Popen(self, mocker):
        mock = mocker.patch('theheck.shells.bash.Popen')
        return mock

    @pytest.fixture(autouse=True)
    def shell_aliases(self):
        os.environ['TF_SHELL_ALIASES'] = (
            'alias fuck=\'eval $(theheck $(fc -ln -1))\'\n'
            'alias l=\'ls -CF\'\n'
            'alias la=\'ls -A\'\n'
            'alias ll=\'ls -alF\'')
    ...
    def test_get_aliases(self, shell):
        assert shell.get_aliases() == {'fuck': 'eval $(theheck $(fc -ln -1))',
                                       'l': 'ls -CF',
                                       'la': 'ls -A',
                                       'll': 'ls -alF'}
    ...
    def test_app_alias(self, shell):
        ...
        assert 'FUCK () {' in shell.app_alias('FUCK')
        assert 'theheck' in shell.app_alias('fuck')
